...
from configuration import DEPTH_SENSOR, TIMESENSOR, PROC_DEPTH_VAR, \
    PROC_LAT_VAR, DEPTH_STATE_SENSOR, PROC_TIME_VAR
from glider_utils import fwd_fill, all_sci_indices
...
class Profiles(object):
    ...
    def find_profiles_by_depth(
            self, ts, depth, tsint=2, winsize=10):
    ...
    def find_profiles_by_depth1(
            self, depth_sensor=DEPTH_SENSOR, tsint=2, winsize=5):
        """Discovery of profiles in a glider segment using depth and time.
        ...
        Configuration.py.
        Depth can be `DEPTH_SENSOR` as configured in configuration.py or
        CTD pressure (Default `DEPTH_SENSOR`). For smoothing a filtered
        depth is created at regular time intervals `tsint` (default
        2 secs) and boxcar filtered with window `winsize` (default 5 points).
        The smoothing is affected by both choice of `tsint` and `winsize`, but
        usually still returns similar profiles.  After profiles are discovered,
        they should be filtered with the `filter_profiles` method of this
        ...
        time_ = self.dba.getdata(TIMESENSOR)
    ...
    def orig_find_profiles_by_depth(self, tsint=2, filter_winsize=10):
        ...
        if PROC_LAT_VAR in self.dba.sensor_names:
            timestamps = self.dba[PROC_LAT_VAR]
        else:
            timestamps = self.dba[TIMESENSOR]

        timestamps = timestamps['data']
        if DEPTH_SENSOR not in self.dba.sensor_names:
            logging.warning(
                ('{:s} not found in dba {:s} for profiles, '
                 'trying pressure/depth instead').format(
                    DEPTH_SENSOR, self.dba.source_file)
            )
            ...
            depth = self.dba[DEPTH_SENSOR]['data']
    ...
    def find_profiles_by_depth_state(self):
        ...
        if DEPTH_STATE_SENSOR not in self.dba.sensor_names:
            logging.debug('Thought there was depth state, but not')
            return profile_indexes

        depth_state = self.dba.getdata(DEPTH_STATE_SENSOR)
